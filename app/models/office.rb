...
class Office < ApplicationRecord
  ...
  scope :volunteer_minutes_by_county, ->(state) { joins(:address, needs: :shifts).where.not(shifts: { user_id: nil }).where(addresses: { state: state }).group('offices.id, addresses.county').sum(:duration) }
  scope :children_served, -> { joins(needs: :shifts).where.not(shifts: { user_id: nil }).sum('DISTINCT ON (needs.office_id) needs.number_of_children') }
