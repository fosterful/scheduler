...
class Blockout < ApplicationRecord
  ...
  validates :start_at, :end_at,
            presence: true
  validates :start_at,
            inclusion: { in:      ->(_blockout) { (Time.zone.now.beginning_of_day..) },
                         message: 'must be in the future' },
            if:        :start_at_changed?

  validates :end_at,
            inclusion: { in:      ->(blockout) { (blockout.start_at..) },
                         message: 'must be beyond start at' },
  ...
  scope :current, lambda {
                    where('start_at < ? AND last_occurrence > ?',
                          15.days.from_now.end_of_day,
