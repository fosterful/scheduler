...
class DateTimePicker extends React.Component {
  ...
  toggleAllDay = _ => {
    const {
      state: { allDay, fromDate, toDate }
    } = this;
    if (allDay) {
      this.setState(state => ({ allDay: false, toDate: toDate }));
    } else {
      this.setState(
        state => ({ allDay: true, fromTime: "00:00", toTime: "23:59" }),
        this.computeResult
      );
    }
  };
  ...
  renderTimeSelect = (input, value) => {
    if (!this.state.allDay) {
      ...
      return (
        <select
          className="conditional-time-select"
          value={value}
  ...
  computeResult = _ => {
    const {
      state: { fromDate, toDate, fromTime, toTime }
    } = this;
    const update = {};

    const startAtStr = `${moment(fromDate).format(
      "YYYY-MM-DD"
    )}T${fromTime}:00`;
  ...
  render() {
    const {
      ...
      handleInputChange,
      renderTimeSelect,
      state: { allDay, fromDate, toDate, fromTime, toTime }
    } = this;
    return (
        ...
        <div className="cell small-12 medium-2">
          <div className="grid-x grid-margin-x align-middle">
            <div className="cell small-4 medium-12">
              <label className="text-right medium-text-center medium-margin-bottom-0 margin-bottom-1">
                All Day
              </label>
            </div>
        ...
        <div className="cell small-12 medium-5">
          <div className="grid-x grid-margin-x align-middle">
            <div className="cell small-4 medium-12">
              <label
                htmlFor=""
                className="text-right medium-text-left medium-margin-bottom-0 margin-bottom-1"
              >
                Start Date
              </label>
            </div>
