...
RSpec.describe Office, type: :model do
  ...
  context 'reporting' do
    ...
    let(:wa_need1) { create(:need_with_shifts, user: wa_sw1, number_of_children: 2, expected_duration: 60, office: wa_office1) }
    ...
    describe '.volunteer_minutes' do
      it 'returns the total volunteer minutes grouped by office' do
        expect(described_class.volunteer_minutes).to eql(or_office.id => 60, wa_office1.id => 60, wa_office2.id => 240)
      end
    end

    describe '.volunteer_minutes_by_state' do
      it 'returns the total volunteer minutes groupe by state' do
        expect(described_class.volunteer_minutes_by_state).to eql('OR' => 60, 'WA' => 300)
      end
    end

    describe '.volunteer_minutes_by_county' do
      it 'returns the total volunteer minutes groupe by county' do
        expect(described_class.volunteer_minutes_by_county('WA')).to eql('Lewis' => 240, 'Clark' => 60)
      end
    end
    ...
    #   it 'does something' do
    #     expect(described_class.children_served_by_county('OR')).to eql(nil)
    #   end
    # end
